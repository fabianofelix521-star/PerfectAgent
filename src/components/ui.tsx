    ...
    <section className="chat-surface app-scrollbar fx-fade-in flex h-full min-h-0 min-w-0 flex-col overflow-x-hidden overflow-y-auto rounded-[22px] border border-white/70 p-3 sm:p-5 lg:rounded-[28px] lg:p-7">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl md:text-4xl">
            {title}
          </h1>
...
export function ToggleRow({
  ...
  return (
    <button
      onClick={onToggle}
      className="fx-press flex w-full items-center justify-between gap-3 rounded-[16px] border border-white/70 bg-white/65 p-3 text-left"
    >
      <span>
        <span className="block text-xs font-semibold text-slate-950 sm:text-sm">
          {title}
        </span>
        {desc ? (
...
export function Tag({ children }: { children: ReactNode }) {
  return (
    <span className="rounded-full bg-slate-950/5 px-2 py-0.5 text-[11px] font-bold text-slate-600">
      {children}
    </span>
  );
}
...
export function Spinner({ size = 14 }: { size?: number }) {
  return (
    <span
      aria-hidden="true"
      className="inline-block animate-spin rounded-full border-2 border-slate-300 border-t-[#17172d]"
...
export function Modal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur sm:p-4"
      onClick={onClose}
    >
      <motion.div
      ...
      >
        ...
        <div className="app-scrollbar max-h-[68vh] overflow-y-auto p-4 sm:p-5">
          {children}
        </div>
        {footer ? (
          <div className="flex flex-wrap items-center justify-end gap-2 border-t border-slate-100/60 bg-slate-50/40 px-4 py-3 sm:px-5">
            {footer}
          </div>
        ) : null}
      </motion.div>
